...
import { patientBloodPressureRegex } from 'api/utils/regex'
import { storageService } from 'services/storage.adapter'
import type { FakePatient } from 'api/data/users'
import type { BloodPressure } from '@/domain/medical'

export const getPatientBloodPressure = () =>
  mockAdapter.onGet(patientBloodPressureRegex).reply(config => {
    logRequest(config)
    const id = config.url?.match(patientBloodPressureRegex)?.[1]
