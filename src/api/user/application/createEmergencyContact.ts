...
export const createEmergencyContact = () =>
  mockAdapter.onPost(emergencyContactRegex).reply(config => {
    logRequest(config)
    ...
    if (patient.emergencyContact) {
      return [
        400,
        { message: 'An emergency contact already exists for this patient' },
      ]
    }

    const payload = JSON.parse(config.data) as Parameters<
      ApiService['createPatientEmergencyContact']
    >[1]
